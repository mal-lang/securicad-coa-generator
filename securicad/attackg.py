...
class AttackGraph(nx.DiGraph):
    ...
    def prettyprint(self, ttc=False):
        ...
        if ttc:
            nx.draw_networkx_labels(self, pos=positioning, labels={
                gnode: self.nodes[gnode]["ttc"] for gnode in self.nodes}, font_size=8)
        else:
            nx.draw_networkx_labels(self, pos=positioning, labels={
                gnode: gnode for gnode in self.nodes}, font_size=8)
        plt.show()
        return
